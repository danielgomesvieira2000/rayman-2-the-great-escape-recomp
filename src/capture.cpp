...
void take(uint8_t* rdram, const char* reason) {
    ...
    if (stub) {
        stub << "# Graphics issue: (one line -- what looks wrong)\n"
             ...
             << "## The frame itself\n"
             << "\n"
             << "Turn on Developer Mode in the Graphics tab and press F1 to open RT64's\n"
             << "frame inspector. Pause on the bad frame and walk the draw calls; a\n"
             << "screenshot of that panel, or just \"draw call N in projection M is the\n"
